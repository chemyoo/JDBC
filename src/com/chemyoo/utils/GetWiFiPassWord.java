...
public class GetWiFiPassWord {
	 ...
	 */
	public static void printWiFiPassWord(String result) throws FileNotFoundException {
		...
		while (scanner.hasNext() && (line = scanner.nextLine()) != null) {
			...
			if (line.contains("SSID 名称")) {
				...
				System.err.println("无线SSID:" + wifi.trim());// trim()去掉多余的空白符
			}
			// 关键内容 : *********
		...
		try {
			...
			while (scanner.hasNext() && (line = scanner.nextLine()) != null) {
				...
				if (line.contains(WiFiNameLineFlag)) {
					wifi = line.substring(line.lastIndexOf(WiFiNameLineFlag) + WiFiNameLineFlag.length(),
							line.lastIndexOf(":"));
					// System.out.print("无线:"+wifi.trim());//trim()去掉多余的空白符
					buff.append("无线SSID:" + wifi.trim() + " |");
				}
				// 关键内容 : *********
				if (line.contains("关键内容")) {
					passworld = line.substring(line.lastIndexOf(":") + 1);
					// System.out.println("|密码:"+passworld.trim());//trim()去掉多余的空白符
					buff.append(" WIFI密码:" + passworld.trim());
				}
			}
		} catch (Exception e) {
			// ignore
	...
	public static String getPassWordByName(String name) {
		String commandStr = "netsh wlan show profile name=" + name + " key=clear";
	...
	public static void main(String[] args) throws FileNotFoundException, InterruptedException {
		...
		for (String string : WiFiNameList) {
			...
			String result = getPassWordByName(string);
			System.out.println(getWiFiMap(result));
			printWiFiPassWord(result);
		}
		// 恢复到原来的标准输出流
		System.setOut(out);
